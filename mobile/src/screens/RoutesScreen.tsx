...
export const RoutesScreen: React.FC = () => {
  ...
  if (isLoadingRoutes && routes.length === 0) {
    return (
      <LoadingState
        variant="screen"
        message="Evaluating multi-objective routes with Chronos-2..."
      />
    );
  }
  ...
  // Previously this screen rendered "0 Scored" and nothing else.
  if (routes.length === 0) {
    return (
      <View style={styles.stateWrapper}>
        <EmptyState
          title="No routes scored"
          message="The engine returned no candidate routes for this corridor."
          icon={<GitFork size={20} color={colors.text.secondary} />}
